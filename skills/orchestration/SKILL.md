...
| Elastic overflow | DeepSeek V4 Flash (OpenRouter) | `deepseek-implementer` agent | **The owned lanes are capped or throttled and work still has to move**, or a fourth independent family is wanted. Pay-per-token and uncapped — see capacity note. Requires the `kimi` CLI as host plus a configured OpenRouter provider. |
| Judgment | Opus 5 | `fable-advisor` agent | Not an implementation lane. Pinned to Opus 5 so a consult is worth the same from any session. See "Commitment boundaries" below. |

Lane capacity and concurrency — measured/observed 2026-07, marked as such; re-measure before treating as constants:

- **grok** (SuperGrok **Heavy**, active since 2026-07-18): ~990M tok/week at ~€0.07/Mtok (Heavy is 10× base quota for 10× price — ~1 token per dollar, so per-token value is unchanged; the ceiling, not the price, is what moved). Fastest lane measured (TTFT p50 6.2s, highest tok/s). **Observed ×9 concurrent sessions without throttling on base; Heavy's real differentiator is ~10× concurrency/compute** — re-measure the concurrent-session ceiling on Heavy, don't assume it's still 9. Internal parallelism (`--best-of-n <N>`, headless-only; inline `--agents`) multiplies further. **This is the least-constrained lane in the fleet and the primary sink for bulk/routine volume** — grok Heavy absorbs essentially all volume shifted off the architect's budget.
...
- **deepseek** (`kimi` CLI hosting OpenRouter, `deepseek/deepseek-v4-flash-0731`): added 2026-08-01. **$0.14 / 1M prompt, $0.28 / 1M completion — pay-per-token and UNCAPPED**, roughly 2.5× grok's per-token subscription rate. It is therefore **not a saving; it is elasticity.** Every other lane stops dead at its cap; this one keeps going at predictable marginal cost. Route here when the owned subscriptions are exhausted or throttled — never while grok or codex still have headroom. Host verified (kimi edits files headless); the OpenRouter pairing is unproven until the operator's key is imported. 1M context, `tool_use` supported.
- **fable-advisor consults**: cheap and parallel-safe — but they bill the same Claude window as the architect itself, as does every other Claude subagent. The CLI lanes never do. Push volume outward.
