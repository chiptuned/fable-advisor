...
| Overflow / third family | Composer 2.5 | `cursor-implementer` agent | Grok lane down or rate-limited and the task doesn't warrant codex's reasoning premium — or you want a third independent diff to race |
...
- **Cursor lane (optional):** the `cursor-implementer` agent needs the [Cursor CLI](https://cursor.com/cli) installed and authenticated (`cursor-agent login`). It invokes **Composer 2.5** headlessly (`cursor-agent -p --model composer-2.5 -f`). Note `-f` force-allows commands — blunter than codex's workspace sandbox — so the agent snapshots `git status` before running and takes only bounded, well-specified tasks. Without the CLI or auth it reports `STATUS: unavailable`.
